...
def wait_for_sso_cookie(timeout=60):
	...
	# 增加超时到 60s，因为注册后页面可能需要跳转才能设置 sso cookie。
	deadline = time.time() + timeout
	last_seen_names = set()
	poll_count = 0

	# 先等几秒让页面完成跳转
	print("[*] 等待页面跳转完成后再检测 sso cookie...")
	time.sleep(3)
		...
		try:
			...
			if poll_count % 10 == 1:
				try:
					...
					print(f"[Debug] sso cookie 轮询 #{poll_count} | URL={current_url} | 已见cookie={sorted(last_seen_names)}")
				except Exception:
					pass
			...
			for item in cookies:
				...
				if name == "sso" and value:
					...
					print(f"[*] 注册完成后已获取到 sso cookie (耗时约{elapsed}s)。")
					return value
...
def run_single_registration(output_path=DEFAULT_SSO_FILE, extract_numbers=False):
	...
	if extract_numbers:
		try:
			...
			print(f"[Warn] 提取数字失败（page 可能已断开）: {e}")
